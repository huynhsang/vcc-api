...
export default (News) => {
    ...
    News.validate('description', function (err) {
        if (this.description && Joi.string().trim().min(News.DEFAULT_MIN_LENGTH).max(News.DEFAULT_MAX_DESC_LENGTH).optional()
    ...
    News.validate('userId', function (err) {
        if (this.userId && Joi.string().hex().length(24).optional().validate(String(this.userId)).error) {
            return err('INVALID');
        }
    }, {
        message: {
